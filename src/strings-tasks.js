 ...
 */
function getStringLength(str = null) {
  if (str !== null && str !== '' && str !== undefined) return str.length;
  return 0;
}
...
function isPalindrome(str) {
  const solidStr = str
    ...
    .replaceAll('?', '')
    .replaceAll(',', '');
  const reversStr = solidStr.split('').reverse().join('');
  return solidStr === reversStr;
}
...
function encodeToRot13(str) {
  ...
  const legends = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm!?';
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    if (alfabet.indexOf(str[i]) !== -1) {
      result += legends[alfabet.indexOf(str[i])];
    } else {
      result += ' ';
    }
  }
